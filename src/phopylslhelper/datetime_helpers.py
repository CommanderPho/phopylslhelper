...
class DateTimeHelpers:
    """Encapsulates datetime conversion and XDF header parsing utilities."""

    @classmethod
    def to_display_timezone(cls, dt: Union[datetime, pd.Timestamp]) -> datetime:
    ...
    @classmethod
    def _to_utc_datetime(cls, dt: Union[datetime, pd.Timestamp]) -> datetime:
    ...
    @classmethod
    def datetime_to_unix_timestamp(cls, dt: Union[datetime, np.ndarray, List[datetime]]) -> Union[float, List[float]]:
        ...
        # Scalar value
        dt_utc = cls._to_utc_datetime(dt)
        return dt_utc.timestamp()


    @classmethod
    def unix_timestamp_to_datetime(cls, ts: Union[float, np.ndarray, List[float]]) -> Union[datetime, List[datetime]]:
    ...
    @classmethod
    def datetime_to_float(cls, dt: Union[datetime, np.ndarray, List[datetime]], reference_datetime: datetime) -> Union[float, List[float]]:
    ...
    @classmethod
    def float_to_datetime(cls, timestamp: Union[float, np.ndarray, List[float]], reference_datetime: datetime) -> Union[datetime, List[datetime]]:
    ...
    @classmethod
    def format_seconds_as_hhmmss(cls, seconds: float, include_fractional: bool = True) -> str:
    ...
    def get_reference_datetime_from_xdf_header(cls, file_header: dict) -> Optional[datetime]:
        ...
        for path in possible_paths:
            try:
                        ...
                        try:
                            ...
                            for fmt in ['%Y-%m-%d %H:%M:%S', '%Y-%m-%dT%H:%M:%S', '%Y-%m-%dT%H:%M:%S.%f']:
                                try:
                                    ...
                                    continue
                            ## END for fmt in ['%Y-%m-%d %H:%M:%S', '%Y-%m-%dT%H:%M:%S', '%Y-%m-%dT%H:%M:%S.%f']...
                ...
                continue
        ## END for path in possible_paths...
    ...
    @classmethod
    def get_earliest_reference_datetime(cls, file_headers: list, datasources: list) -> Optional[datetime]:
        ...
        for file_header in file_headers:
            ref_dt = cls.get_reference_datetime_from_xdf_header(file_header)
            if ref_dt is not None:
                reference_datetimes.append(ref_dt)
        ## END for file_header in file_headers...
    ...
    @classmethod
    def _normalize_datetime_to_utc_naive(cls, series: pd.Series) -> pd.Series:
        """
...
to_display_timezone = DateTimeHelpers.to_display_timezone
_to_utc_datetime = DateTimeHelpers._to_utc_datetime
datetime_to_unix_timestamp = DateTimeHelpers.datetime_to_unix_timestamp
unix_timestamp_to_datetime = DateTimeHelpers.unix_timestamp_to_datetime
datetime_to_float = DateTimeHelpers.datetime_to_float
float_to_datetime = DateTimeHelpers.float_to_datetime
format_seconds_as_hhmmss = DateTimeHelpers.format_seconds_as_hhmmss
get_reference_datetime_from_xdf_header = DateTimeHelpers.get_reference_datetime_from_xdf_header
get_earliest_reference_datetime = DateTimeHelpers.get_earliest_reference_datetime
_normalize_datetime_to_utc_naive = DateTimeHelpers._normalize_datetime_to_utc_naive
